...
- 增加一个轻量 Web 层重试队列，补发本地 `state.records` 里仍可同步的 pending 或尚未标记同步状态的照片。
...
- `collectPendingPhotoSyncs()`：扫描 `state.records` 的 raw `photos`，收集 `syncStatus !== "synced"` 且 `photo.url` 是 `data:image/...` 的照片。
...
一次同步失败后会给照片累加 `syncAttempts`，同一浏览器会话最多自动短重试 3 次。之后仍保留 `pending` 和 `syncError`，等待下一次 app 初始化、网络恢复或手动重试入口继续处理，避免 API 长时间不可用时形成过密请求循环。
